...
exports.ContractVerifier = {
    async getSourcesJsonUrl(codeCellHash, options) {
        ...
        const { result: itemAddRes } = await tc.runMethod(seqno, isTestnet ? SOURCES_REGISTRY_TESTNET : SOURCES_REGISTRY, "get_source_item_address", args.build());
    ...
    async getSourcesData(sourcesJsonUrl, options) {
        ...
        const verifiedContract = await (await fetch(ipfsConverter(sourcesJsonUrl, isTestnet))).json();
