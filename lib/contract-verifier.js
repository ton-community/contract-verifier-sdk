...
const SOURCES_REGISTRY_TESTNET = ton_1.Address.parse("EQCsdKYwUaXkgJkz2l0ol6qT_WxeRbE_wBCwnEybmR0u5TO8");
...
exports.ContractVerifier = {
    async getSourcesJsonUrl(codeCellHash, options) {
        ...
        args.writeNumber(bigIntFromBuffer(toSha256Buffer(options?.verifier ?? "verifier.ton.org")));
        args.writeNumber(bigIntFromBuffer(Buffer.from(codeCellHash, "base64")));
        const { result: itemAddRes } = await tc.runMethod(seqno, options.testnet ? SOURCES_REGISTRY_TESTNET : SOURCES_REGISTRY, "get_source_item_address", args.build());
